...
export class DialogComponent implements OnInit {
  ...
  platform: any;
  branch: any;
  constructor(private ApiService: ServerService, private route: ActivatedRoute, private router: Router, private titleService: Title) { }
  ...
  sortData(D: any) {
    let stages: any[] = []
    let obj: any[] = []
    this.successJobs = D.jobs.filter((job: any) => job.status == 'success').length
    this.failedJobs = D.jobs.filter((job: any) => job.status == 'failed').length
    ...
    stages.forEach(stage => {
      const stageObj = {
        ...
        allJobs: D.jobs.filter((job: any) => job.stage == stage),
        status: this.stageStatusForObj(stage, D.jobs),
        noOfSuccessJobs: D.jobs.filter((job: any) => job.stage == stage).filter((job: any) => job.status == "success").length,
        noOfFailedJobs: D.jobs.filter((job: any) => job.stage == stage).filter((job: any) => job.status == "failed").length
      };
      obj.push(stageObj);
    })
    this.pipeD = obj
  }
  stageStatusForObj(stage: string, jobs: any[]) {
