...
export class DoughnutGraphComponent implements OnInit {
  ...
  jobStatusCount(data: any, status: any) {
    var passed = 0;
  ...
  getJobPercentForPipeline(data: any):any {
    try {
      if (data.status == "success") {
        ...
        let totalJobs = data.jobs.filter((j:any) => j.status == 'success' || j.status == 'failed')
  ...
  getJobPercentForPipelineDoughnut(data: any, returnFor: any):any {
    try {
      if (data.status == "success") {
        return "99 1";
      }
